...
namespace TareasMVC.Controllers
{
    ...
    public class ArchivosController : ControllerBase
    {
            ...
            [FromForm] IEnumerable<IFormFile> archivos)
        {
            ...
            var existenArchivosAdjuntos = await context.ArchivoAdjuntos.AnyAsync(a => a.TareaId == tareaId);
        ...
        public async Task<IActionResult> Ordenar(int tareaId, [FromBody] Guid[] ids)
        {
            ...
            var archivosId = archivosAdjuntos.Select(x => x.Id);

            var idsArchivosNoPertenecenALaTarea = ids.Except(archivosId).ToList();
            if (idsArchivosNoPertenecenALaTarea.Any())
            {
                return BadRequest("No todos los pasos están presentes.");
            }

            var archivoDiccionario = archivosAdjuntos.ToDictionary(p => p.Id);

            for (int i = 0; i < ids.Length; i++)
            {
                var archivoId = ids[i];
                var archivo = archivoDiccionario[archivoId];
                archivo.Orden = i + 1;
            }
